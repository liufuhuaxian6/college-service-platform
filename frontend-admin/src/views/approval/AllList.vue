      ...
      <el-table-column label="操作" width="180" fixed="right">
        <template #default="{ row }">
          ...
          <el-button
            link
            type="warning"
            v-if="row.status === 'approved' && !row.downloadedAt"
            @click="handleWithdraw(row)"
          >
            撤回
          </el-button>

          <el-tag type="info" v-if="row.status === 'downloaded'">
            已锁定
          </el-tag>
        </template>
      </el-table-column>
...
const statusMap = {
  ...
  downloaded: { label: '已锁定', type: 'info' },
}
