...
-- 学生提交的入党/入团流程申请
CREATE TABLE party_process_application (
    id             BIGSERIAL PRIMARY KEY,
...
CREATE INDEX idx_party_app_user ON party_process_application(user_id);
CREATE INDEX idx_party_app_template ON party_process_application(template_id);
CREATE INDEX idx_party_app_status ON party_process_application(status);
...
-- 智能问答知识库默认条目 (关键词匹配, 无需 embedding; 覆盖证明办理 / 党团流程常见问题)
INSERT INTO qa_knowledge (category, question, answer, keywords, sort_order) VALUES
('证明办理', '在读证明如何申请？',
