...
- **RAG 智能问答**：当前开发/生产方案使用 PostgreSQL + pgvector 作为向量数据库，TEI HTTP 服务生成 512 维 BGE-small-zh-v1.5 向量。问答链路为：标准知识库关键词命中 → RAG 向量召回 + 关键词/意图重排 → AI Provider 或抽取式回答 → manual 兜底。校历、节假日、报到、学期安排等短问题会做检索查询扩展并优先匹配 `校历安排` 分类。
- **证明申请模板**：党员证明、团员证明不在运行时解析 docx。模板文件只作为离线参考，后端通过 `CertTemplateRegistry` 固化正文结构和必填字段；小程序提交申请时先读取 `/api/approval/templates/{id}/fields`，再提交 `templateDocId + formData`，审批通过后按固定版式生成 PDF。
- **审批锁定规则**：申请支持草稿、待审批、已通过、已驳回、已撤回、已下载锁定。学生下载证明后状态变为 `downloaded`，锁定归档，禁止撤回和重新审批；预览接口不会锁定。
- **通知与邮箱**：管理端可按角色、年级、专业、班级群发站内通知、邮件、短信模拟。用户邮箱可显式维护；为空时后端按 `学号@ruc.edu.cn` 派生。SMTP 授权码缺失或失败时邮件渠道自动降级为 `email_sim`，不影响站内通知。
- **小程序开发 API**：小程序开发环境默认请求 `http://localhost:8080/api`，生产环境请求 `http://10.10.0.27/api`，也可以通过 `VITE_API_BASE_URL` 覆盖。
- **前端设计（v3）**：管理端与小程序端统一采用**人大红主题**（Pantone 201C `#9D2235`）+ 暖纸灰中性色 + 鎏金点缀，整体风格庄重典雅；两端均融入中国人民大学元素（`RucSeal` 真实校徽组件 + 深红横幅校徽水印、中英文校名、"实事求是"校训、衬线标题、红金双色装饰条）。设计令牌集中在 `frontend-admin/src/assets/styles/theme.scss` 与 `frontend-mp/src/styles/theme.css`，改主题只改这两个文件。
  - **管理端信息架构**：人大红顶部品牌栏 + 浅色分组扁平侧边导航；「待审批/全部申请」合并为**审批中心**（`/approval/center`，抽屉内直接审批，旧路由自动重定向）；首页为**工作台**（待办可页内直接通过/驳回 + 快捷入口 + 图表）；党团流程模板为卡片网格、知识库为分类标签条筛选。登录页为全屏人大红仪式感设计。
  - **小程序交互**：首页为服务大厅（搜索式问答入口 + 我的事项速览 + 服务网格）；提交申请为**三步向导**（选模板→填写→确认）；我的申请带状态筛选标签；党团详情为阶段手风琴；智能问答为沉浸式聊天（欢迎首屏推荐问题 → 对话流）。
  - 注意：**管理端登录角色为 1 级院领导 / 2 级老师 / 3 级班团骨干（骨干仅"学生信息"页），4 级学生走小程序**。
...
`schema.sql` 执行后自动创建（密码均为 `admin123`）：

| 学号 | 角色 | 备注 |
|---|---|---|
| admin | 院领导（1 级） | 全部菜单 |
| T2024001 / T2024002 / T2024003 | 管理老师（2 级） | 无年级/专业/班级 |
| L2024001 / L2024002 / L2023001 | 学生骨干（3 级） | 只看本班数据 |
| 20240001 | 普通学生（4 级） | 走小程序登录 |

> 老师 / 骨干账号用于演示「按身份筛选」「通知群发分角色」等场景。若旧库没有，可单独执行 `deploy/sql/seed_teacher_leader.sql` 补种（幂等）。
...
**节假日 / 校历类问题未匹配**
确认校历文本已作为政策文档导入并完成「重新索引」，分类建议设为 `校历安排`。当前 RAG 会对“节假日、放假、调休、寒暑假、开学、报到、考试周”等意图做查询扩展；如果仍未命中，优先检查 `qa_document_chunk` 是否有对应切片。

**提交党员/团员证明时报“系统内部错误”**
确认办公模板记录标题包含 `党员证明模板` 或 `团员证明模板`，并且小程序提交前已经成功加载模板字段。当前后端不读取 docx 原文件，字段和正文来自 `CertTemplateRegistry`。
...
- [**测试使用说明**](docs/TESTING-GUIDE.md) — 给老师 / 评审 / 同班同学的"上手就测"指南
